...
function getNextFriday(date) {
  ...
  d.setDate(d.getDate() + (5 - (day < 5 ? day : day - 7)));
  return d;
}
...
function getCountWeekendsInMonth(month, year) {
  const curMonth = new Date(Date.UTC(year, month - 1));
  const nextMonth = new Date(Date.UTC(year, month));
  const diff = (nextMonth - curMonth) / 86400000;
  let count = 0;

  for (let i = 1; i <= diff; i += 1) {
    const day = new Date(curMonth.setDate(i)).getDay();
...
function getWeekNumberByDate(date) {
  const firstDay = new Date(Date.UTC(date.getFullYear()));
  const diff = Math.ceil((date - firstDay) / 86400000 + 1);
...
function getNextFridayThe13th(date) {
  for (let i = 0; i <= 11; i += 1) {
    const every13th = new Date(date.getFullYear(), i, 13);
    if (every13th.getDay() === 5) {
      return every13th;
    }
  }
  return undefined;
